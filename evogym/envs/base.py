...
class BenchmarkBase(EvoGymBase):
    ...
    def modular_ob_wrapper_padding(self, origin_ob, index_array, body):
        ...
        modular_obs.extend(obs_padding)

        del modular_obs_tmp
        return np.array(modular_obs).flatten()

    def get_token_metadata(self, body):
        body_flat = np.asarray(body).flatten()
        nonempty_idx = np.where(body_flat != 0)[0]
        empty_idx = np.where(body_flat == 0)[0]
        token_index = np.concatenate((nonempty_idx, empty_idx)).astype(float)
        voxel_type = body_flat[token_index.astype(int)].astype(float)
        rows, cols = np.unravel_index(token_index.astype(int), body.shape)
        grid_pos = np.stack((cols, rows), axis=1).astype(float)
        return voxel_type, grid_pos.flatten(), token_index

    def daab_ob_wrapper_padding(self, pos_ob, vel_ob, index_array, body):
        body_flat = np.asarray(body).flatten()
        robot_pos_com = np.mean(
            self.object_pos_at_time(self.get_time(), "robot"), axis=1
        )
        robot_vel_com = np.mean(
            self.object_vel_at_time(self.get_time(), "robot"), axis=1
        )
        daab_obs = []
        obs_padding = []
        for flat_idx, corner_idx in enumerate(index_array):
            if body_flat[flat_idx] == 0 or np.sum(corner_idx) == -4:
                obs_padding.append(np.zeros(12).astype(float))
                continue
            corners_pos = pos_ob[:, corner_idx].T
            corners_vel = vel_ob[:, corner_idx].T
            voxel_center = np.mean(corners_pos, axis=0)
            voxel_vel = np.mean(corners_vel, axis=0)
            token = np.concatenate(
                (
                    (corners_pos - voxel_center).reshape(-1),
                    voxel_center - robot_pos_com,
                    voxel_vel - robot_vel_com,
                ),
                axis=0,
            )
            daab_obs.append(token)
        daab_obs.extend(obs_padding)
        return np.array(daab_obs).flatten()
    ...
    def set_modular_attributes(self, body=None, nca_setting=None, init_nca_design=None):
        ...
        self.voxel_type, self.grid_pos, self.token_index = self.get_token_metadata(self.body)
        self.init_modular_obs = self.get_modular_obs()
        self.init_modular_obs['stage'] = np.array([1.0 if self.stage == 'act' else 0.0])
        if self.nca_setting is not None:
            self.init_modular_obs['design'] = self.make_design_batch(self.cur_nca_design)
    ...
    def get_modular_obs(self):
        ...
        vel_ob = self.object_vel_at_time(self.get_time(), "robot")
        obs["modular"]= self.modular_ob_wrapper_padding(origin_ob=origin_ob,index_array=self.obs_index_array,body=self.body)
        obs["daab_modular"] = self.daab_ob_wrapper_padding(
            pos_ob=origin_ob, vel_ob=vel_ob,
            index_array=self.obs_index_array, body=self.body)
        obs["other"] = self.get_other_obs()
        obs["act_mask"] = self.act_mask
        obs["obs_mask"] = self.obs_mask
        obs["voxel_type"] = self.voxel_type
        obs["grid_pos"] = self.grid_pos
        obs["token_index"] = self.token_index
        return obs
