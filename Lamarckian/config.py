...
from Lamarckian.globals import CONFIG_FILE_NAME
...
class Config(BaseModel):
    ...
    def initialize(cls):
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="maximum number of robots trained concurrently (default: 1)",
        )
        parser.add_argument(
            "--cuda-devices",
            default="0",
            help="comma-separated CUDA device IDs used to distribute parallel robots (default: 0)",
        )
        parser.add_argument(
            "--no-cuda",
            action="store_true",
            default=False,
            help="disable CUDA and train controllers on CPU",
        )
        parser.add_argument(
            "--crossover_rate",
            type=float,
        ...
        if config.parallel_robots < 1:
            parser.error("--parallel-robots must be at least 1")
        try:
            device_ids = [int(device_id) for device_id in config.cuda_devices.split(",")]
        except ValueError:
            parser.error("--cuda-devices must be comma-separated integer device IDs")
        if not device_ids:
            parser.error("--cuda-devices must contain at least one device ID")

        config.exp_dir.mkdir(parents=True)
        config.save()
