...
from Lamarckian.a2c_ppo_acktr.model import Policy  # type: ignore
from Lamarckian.config import Config
from Lamarckian.globals import JSON_FILE_NAME
from Lamarckian.inherit import inherit_controller_mutation
from Lamarckian.ppo import run_ppo
from Lamarckian.ppo.envs import make_vec_envs
from Lamarckian.ppo.run import ACTOR_CRITIC_FILE_NAME
from Lamarckian.structure import Structure
from Lamarckian.utils import LogWriter
...
def train_individual_worker(
    individual: Individual,
    config: Config,
    parents: Optional[List[Individual]],
) -> Tuple[int, float]:
    """Train one individual in a spawned worker and return its fitness."""

    device_ids = [int(device_id) for device_id in config.cuda_devices.split(",")]
    worker_config = config.copy(
        update={"cuda_device": device_ids[individual.id_ % len(device_ids)]}
    )
    actor_critic = get_controller(
        individual.structure, parents, worker_config, individual.crossover_info, None
    )
    fitness = run_ppo(
        individual.structure, individual.saving_dir, worker_config, actor_critic
    )
    return individual.id_, fitness
