...
from Lamarckian.config import Config
from Lamarckian.globals import POP_CSV_FILE_NAME, POP_TXT_FILE_NAME
from Lamarckian.individual import Individual, train_individual_worker
from Lamarckian.structure import Structure, mutate_structure
from Lamarckian.utils import FitnessWriter, IndexableList, LogWriter
...
class Population:
    ...
    def train(self) -> EvolutionState:
        ...
        # ``max_evaluations`` is a hard total budget, not an off-by-one limit.
        ...
        remaining = self.config.max_evaluations - self.num_evals
        jobs = []
        for individual in self.individual_list:
            ...
            if individual.info.parents_id is None:
                ...
                parents = tuple(
                    self.individual_list[parent_id]
                    for parent_id in individual.info.parents_id
                )
            jobs.append((individual, parents))
            self.log_writer.print_and_write(
                f"training robot {id_} (parents: {individual.parents_id})..."
            )

        if self.config.parallel_robots == 1:
            results = []
            for individual, parents in jobs:
                individual.train(self.config, parents, self.log_writer)
                results.append((individual.id_, individual.fitness))
        else:
            context = multiprocessing.get_context("spawn")
            with ProcessPoolExecutor(
                max_workers=min(self.config.parallel_robots, len(jobs)),
                mp_context=context,
            ) as executor:
                results = list(
                    executor.map(
                        train_individual_worker,
                        (individual for individual, _ in jobs),
                        (self.config for _ in jobs),
                        (parents for _, parents in jobs),
                    )
                )

        for id_, fitness in results:
            individual = self.individual_list[id_]
            individual.info.fitness = fitness
            individual.save()
            self.log_writer.print_and_write(f"\tterminated. fitness: {fitness}")
            self.num_evals += 1
    ...
    def evolve(self):
        ...
        while True:
            ...
            if (
                evolution_state == EvolutionState.END
                or self.num_evals >= self.config.max_evaluations
            ):
