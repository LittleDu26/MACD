...
def run_ppo(
    ...
    args = get_args()
    args.eval_interval=termination_condition.max_iters
    ...
    # device = torch.device(f"cuda:{device_num}" if args.cuda else "cpu")
    device = torch.device("cpu")
    envs = make_vec_envs(args.env_name, structure, seed, args.num_processes,
    ...
    for j in range(num_updates):
        ...
        if (args.eval_interval is not None and len(episode_rewards) > 1
            ...
            determ_avg_reward = evaluate(args.num_evals, actor_critic, obs_rms, args.env_name, structure,seed,
