...
def get_modular_observation_spec(env):
    """Infer CuCo's token shape from the active EvoGym modular observation.

    CuCo's paper uses nine values per voxel: eight kinematic values plus the
    voxel material.  The standard project environment therefore returns nine
    values.  The dimension is nevertheless inferred here so an older local
    EvoGym fork that omits the material value can still be inspected without
    creating a shape error; this compatibility path does not change the
    standard CuCo experiment configuration.
    """
    obs = env.get_modular_obs()
    modular = np.asarray(obs["modular"])
    obs_mask = np.asarray(obs["obs_mask"])
    if modular.ndim != 1 or obs_mask.ndim != 1 or obs_mask.size == 0:
        raise ValueError("EvoGym modular observation must contain flat modular and obs_mask arrays")
    if modular.size % obs_mask.size:
        raise ValueError(
            "modular observation size {} is not divisible by its {} tokens".format(
                modular.size, obs_mask.size
            )
        )
    return modular.size // obs_mask.size, obs_mask.size, obs

def run(env_name, seed, target_design_size, pop_size, train_iters, ac_type=None,
        device_num=1, rl_only=None, save_root="saved_data"):
    ...
    use_cuda = device_num is not None and int(device_num) >= 0 and torch.cuda.is_available()
    ppo_args.cuda = use_cuda
    ppo_args.cuda_deterministic = use_cuda
    ...
    modular_state_dim, sequence_size, obs_sample = get_modular_observation_spec(train_env)
    ...
    home_path = os.path.join(root_dir, save_root, env_name, experiment_name)
    try:
