...
def make_vec_envs(env_name,
    ...
    if len(envs) > 1:
        # The workspace's VecEnv implementation does not expose the optional
        # timeout parameter used by the server fork.  Its workers already
        # close cleanly through the standard VecEnv protocol, so keep the
        # public CuCo API while using the compatible constructor here.
